		...
		else
		{
			...
			else
			{
				bSaveJump = false;
                        ...
                        //Add 'weight' to the jump/freefall by tweaking Pawn gravity and velocity - DMR
                        if(Pawn.Physics == PHYS_Falling)
                        {
                            // Get Gravity
                          addedGravity = Pawn.GetGravityZ();
                            // Scale acceleration to velocity
                          addedGravity *= 1.5;
                            // addedGravity as a percent of Pawn.GetGravityZ()
                          addedGravity *= 2.5;
                          Pawn.Velocity.Z += addedGravity * DeltaTime;
                            // Clamp Y velocity while in the air to limit jump distance
                          Pawn.Velocity.Y=Clamp(Pawn.Velocity.Y, -630, 630);
                        }
